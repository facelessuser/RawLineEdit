...
CSS = '''
div.raw-line-edit { padding: 10px; margin: 0; }
.raw-line-edit h1, .raw-line-edit h2, .raw-line-edit h3,
.raw-line-edit h4, .raw-line-edit h5, .raw-line-edit h6 {
    {{'.string'|css}}
}
.raw-line-edit blockquote { {{'.comment'|css}} }
.raw-line-edit a { text-decoration: none; }
'''
...
class RawLineEditSupportInfoCommand(sublime_plugin.ApplicationCommand):
    """Support info."""
...
class RawLineEditOpenSiteCommand(sublime_plugin.ApplicationCommand):
    """Open site links."""
...
class RawLineEditDocCommand(sublime_plugin.WindowCommand):
    """Open doc page."""
    ...
    def run(self, page):
        ...
        if not has_phantom_support:
            ...
            view.set_name('RawLineEdit - Quick Start')
...
class RawLineEditChangesCommand(sublime_plugin.WindowCommand):
    """Changelog command."""
    ...
    def run(self):
        ...
        text = sublime.load_resource('Packages/RawLineEdit/CHANGES.md')
        view = self.window.new_file()
        view.set_name('RawLineEdit - Changelog')
        view.settings().set('gutter', False)
        view.settings().set('word_wrap', False)
        if has_phantom_support:
            mdpopups.add_phantom(
